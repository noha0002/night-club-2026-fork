  ...
  return (
    <div className="bg-black text-white">
      ...
      <div className="lg:hidden">
        <h2 className="text-white text-2xl font-bold text-center">{songs[current].title}</h2>
        <AudioPlayer ref={playerRef} src={songs[current].src} autoPlay={false} />
          ...
          >
            {songs.map((song, index) => (
              <div key={song.id} className="relative h-[250px] w-full">
                ...
                <button onClick={handlePlay} className="absolute inset-0 flex items-center justify-center text-white text-6xl z-10">
                  {playing ? "⏸" : "▶"}
                </button>
              </div>
            ))}
      ...
      <div className="hidden lg:block">
        <div className="flex">
          <div className="relative w-[300px] h-[200px] shrink-0">
            <Image src={songs[current].cover} alt={songs[current].title} fill className="object-cover" />
          </div>
          <div className="flex-1 flex flex-col justify-center px-6">
            <h2 className="text-white text-2xl font-bold mb-4">{songs[current].title}</h2>
            <AudioPlayer ref={playerRef} src={songs[current].src} autoPlay={false} />
          </div>
        </div>
        ...
        <div className="slide-container relative mt-2">
          ...
          >
            {songs.map((song, index) => (
              <div key={song.id} className="relative h-[180px] mx-1 cursor-pointer group" onClick={() => setCurrent(index)}>
                <Image src={song.cover} alt={song.title} fill className="object-cover" />
                ...
                <button onClick={handlePlay} className="absolute inset-0 flex items-center justify-center z-10 opacity-0 lg:group-hover:opacity-100 transition-opacity duration-300">
                  <span className="text-[#FF2A70] text-5xl border-2 border-[#FF2A70] rounded-full w-16 h-16 flex items-center justify-center">{playing ? "⏸" : "▶"}</span>
                </button>
                <p className="absolute bottom-2 left-2 text-white text-xs z-10">{song.title.substring(0, 10)}...</p>
              </div>
            ))}
          </Slide>
